...
export function ChatRuntimeProvider({ children }: { children: React.ReactNode }) {
  ...
  const isRunning = activeSession
    ? activeSession.messages.some((m) => m.status === 'pending' || m.status === 'streaming')
    : false
  ...
  const threadMessages = useMemo(() => {
    ...
    for (const msg of messages) {
        ...
        if (!msg.toolCalls || msg.toolCalls.length === 0) {
          ...
          for (let i = result.length - 1; i >= 0; i--) {
            ...
            if (rm.role === 'assistant') {
              const part = rm.content.find(
                (c: any) => c.type === 'tool-call' && c.toolCallId === msg.toolCallId
              )
      ...
      if (text.includes('<think') || text.includes('</think>')) {
        const thinkMatches = [
          ...text.matchAll(/<think(?:\s+duration="(\d+)")?>([\s\S]*?)<\/think>/g)
        ]
      ...
      if (reasoningText) {
        content.push({
          ...
          text: reasoningText,
          duration:
            msg.reasoningDuration !== undefined
  ...
  const checkProfilesOrToast = () => {
    ...
    const session = activeSid
      ? useChatStore.getState().sessions.find((s) => s.id === activeSid)
      : null
    ...
    if (!displayModelId) {
      toast({
        ...
        description: t(
          ...
          '您需要先前往设置添加 AI 供应商并选择一个对话模型才能开始。'
        )
      })
      return false
    }
