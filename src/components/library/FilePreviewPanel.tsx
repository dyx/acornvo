...
export function FilePreviewPanel(): JSX.Element {
  ...
  if (!selectedPath || !detail) {
    return (
      <div data-testid="preview-empty" className="flex flex-1 items-center justify-center text-sm text-[color:var(--color-ink-3)]">
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-[640px] px-8 py-6">
        <div className="mb-3 flex items-center gap-2 font-mono text-[11px] text-[color:var(--color-ink-3)]">
        ...
        {summary.has_summary && frontmatter.summary ? (
          <div className="mb-5 rounded-[10px] border-[0.5px] border-[color:var(--color-line)] bg-[color:var(--color-paper-2)] px-4 py-4">
          ...
          <div data-testid="preview-reviewing-loader"
            className="mb-5 flex items-center gap-2.5 rounded-[10px] border-[0.5px] border-dashed border-[color:var(--color-acorn)] bg-[color:var(--color-acorn-bg)] p-4">
        ...
        <Button onClick={() => navigate(`/editor/${encodeURIComponent(summary.path)}`)}
          className="serif inline-flex items-center gap-2 bg-[color:var(--color-acorn)] text-[oklch(0.98_0.01_60)]">
