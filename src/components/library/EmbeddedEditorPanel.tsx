...
function EditorPropertiesPanel(props: {
  ...
  const { onOpenAiDrawer, onTriggerReview } = props
        ...
        <div className="mt-4">
          {fm.summary || fm.ai_summary ? (
            ...
            <div
              ...
              className="flex items-center gap-2.5 rounded-[10px] border-[0.5px] border-dashed border-[color:var(--color-acorn)] bg-[color:var(--color-acorn-bg)] p-4"
            >
              <span className="h-3 w-3 animate-spin rounded-full border-[1.5px] border-[color:var(--color-acorn)] border-t-transparent" />
              <span className="serif text-[13px]">
                {t('library.reviewing')} · {profileName ?? 'AI'}
              </span>
            </div>
          ) : summary.review_status === 'pending' ? (
            <div
              data-testid="preview-pending-loader"
              className="flex items-center gap-2.5 rounded-[10px] border-[0.5px] border-dashed border-[color:var(--color-line)] bg-[color:var(--color-paper-2)] p-4"
            >
              <span className="serif text-[13px] text-[color:var(--color-ink-4)]">
                {t('library.review_pending')}
              </span>
            </div>
          ) : summary.review_status === 'failed' ? (
            <div
              data-testid="preview-review-failed"
              className="flex items-center gap-2.5 rounded-[10px] border-[0.5px] border-dashed border-[color:var(--color-berry)] bg-[color:var(--color-paper-2)] p-4"
            >
              <span className="text-[13px]">⚠️</span>
              <span className="serif text-[13px] text-[color:var(--color-berry)]">
                {t('library.review_failed')}
              </span>
            </div>
          ) : summary.rating === null ? (
          ...
          {summary.tags.length > 0 ? (
            <div className="mt-4 flex flex-wrap gap-1.5">
              {summary.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border-[0.5px] border-[color:var(--color-line)] bg-[color:var(--color-leaf-bg)] px-2.5 py-0.5 font-mono text-[11px] text-[color:var(--color-ink-2)]"
                >
                  #{tag}
                </span>
              ))}
