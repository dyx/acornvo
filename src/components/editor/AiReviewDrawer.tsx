...
export function AiReviewDrawer(props: AiReviewDrawerProps) {
  ...
  return (
    <aside className="w-[360px] h-full bg-background border-l p-6 overflow-y-auto" role="dialog" aria-label={t('editor.ai.drawer.title')}>
      <header className="flex items-center justify-between pb-4 border-b">
        <h2 className="text-lg font-medium">{t('editor.ai.drawer.title')}</h2>
        <Button variant="ghost" size="icon" onClick={props.onClose} aria-label={t('common.close')}>×</Button>
      </header>
      ...
      <section className="mt-6 space-y-2">
        ...
        <Button variant="outline" size="sm" onClick={props.onUseTitle}>{t('editor.ai.useAsTitle')}</Button>
      </section>
      ...
      <section className="mt-6 space-y-2">
        ...
        <ul className="flex flex-wrap gap-2">
          {tags.map(tg => <li key={tg} className="rounded-full border bg-muted/50 px-2 py-0.5 text-xs">#{tg}</li>)}
        </ul>
        <Button variant="outline" size="sm" onClick={props.onMergeTags}>{t('editor.ai.mergeTags')}</Button>
      </section>
      ...
      <footer className="mt-8 flex flex-wrap gap-2">
        ...
        <Button variant="outline" onClick={props.onReject}>{t('editor.ai.reject')}</Button>
        {props.clipId !== null && (
          <Button variant="ghost" onClick={props.onRerun}>{t('editor.ai.rerun')}</Button>
        )}
      </footer>
