...
import {
  useFileMentionAdapter,
  useFileMentionStore
} from '@/components/assistant-ui/file-mention-adapter'
    ...
    >
      ...
      >
        <div className="mx-auto flex w-full max-w-(--thread-max-width) flex-1 flex-col px-4">
          ...
          <div data-slot="aui_message-group" className="pb-10 flex flex-col gap-y-8 empty:hidden">
        ...
        >
          ...
          <ComposerPrimitive.Input
            placeholder={t(
              ...
              'Type a message... (Type @ to mention files, Cmd+Enter to send)'
            )}
    ...
    <div className="aui-composer-action-wrapper relative flex items-center justify-between mt-1">
      ...
      <div className="flex items-center gap-1">
        <ContextDisplay.Ring
          ...
          usage={lastAssistantMessage?.usage as any}
        />
...
const AssistantMessage: FC = () => {
  ...
  const originalMessage = useChatStore((s) =>
    s.activeSessionId
      ? s.bySession[s.activeSessionId]?.messages.find((m) => String(m.id) === messageId)
      : null
  )
        ...
        <MessagePrimitive.GroupedParts
          groupBy={(part) => {
            ...
            if (part.type === 'text') {
              ...
              if (index === -1) return []
              const hasToolCallAfter = parts
                .slice(index + 1)
                .some((p: any) => p.type === 'tool-call')
              const hasReasoningAfter = parts
                .slice(index + 1)
                .some((p: any) => p.type === 'reasoning')
              if (hasToolCallAfter || hasReasoningAfter)
                return ['group-chainOfThought', 'group-text']
              return []
            }
            return []
          }}
  ...
  return (
    ...
    >
      <div className="aui-user-message-content-wrapper relative col-start-2 min-w-0 flex flex-col gap-2 items-end">
        <MessagePrimitive.GroupedParts
          groupBy={(part) => (part.type === 'file' ? ['group-file'] : ['group-content'])}
        >
          {({ part, children }) => {
...
const UserActionBar: FC = () => {
  ...
  const isLatestUserMessage = useChatStore((s) => {
    ...
    return (
      userMessages.length > 0 && userMessages[userMessages.length - 1].id.toString() === messageId
    )
  })
