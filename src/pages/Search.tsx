...
function SearchResults({ q }: { q: string }): JSX.Element {
  ...
  if (q.length === 0) {
    return (
      <div className="flex flex-col gap-4">
        <div className="text-sm text-muted-foreground">{t('search.empty_q')}</div>
  ...
  if (pending) {
    return <div className="text-sm text-muted-foreground">{t('search.pending')}</div>
  }

  if (syntaxError) {
    return <div className="text-sm text-destructive">{t('search.syntax_error')}</div>
  }

  if (items.length === 0) {
    return <div className="text-sm text-muted-foreground">{t('search.no_results_full')}</div>
  }
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-border p-4">
        ...
        />
        <div className="mt-1 text-xs text-muted-foreground">{t('search.phrase_hint')}</div>
      </div>
