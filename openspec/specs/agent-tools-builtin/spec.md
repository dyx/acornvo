...
Tool `clip_summary` SHALL 接受 `{ clipId: number, force?: boolean }`；调 phase 15 `reviewer.reviewClip(clipId, { force })`；返回 `{ summary, suggestedTitle, tags, keyQuotes, reviewedAt }`。sideEffect=false（reviewer 内部会写 frontmatter，但它是幂等 + 对内部字段 ai_* 的写入；视为"让 AI 生成"而非"agent 修改用户内容"）。
