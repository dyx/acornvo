...
- [x] 9.1 首次启动（无 `~/.acornvo/recent-projects.json`）→ 显示 Picker 空态
- [x] 9.2 新建树林 → 目录结构正确（`.acornvo/project.json` / `inbox/` / `assets/` / `.nosync` / `.icloud`）
- [x] 9.3 关闭应用后重启 → 自动进入最近树林
- [x] 9.4 同一树林启动第二个 Acornvo 实例 → 第二个显示接管确认；接管后第一个失效
- [x] 9.5 把树林目录手动移动到新位置 → 最近列表里该项变失效，点"移除"消失
- [x] 9.6 打开 Obsidian vault → `.acornvo/` 被自动创建，`.obsidian/` 未被动
- [x] 9.7 树林路径含 `iCloud` → `project.json.sync_warning` = `"iCloud"`，日志有警告
- [x] 9.8 TitleBar 切换树林菜单可用；切换时发出 `project:changed` 事件
