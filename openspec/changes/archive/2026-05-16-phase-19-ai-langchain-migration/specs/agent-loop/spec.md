...
**Reason**: 原约束是手写 loop 的折中（同步执行 + 状态管理简单），非产品需求。LangGraph v1 默认并行执行所有 tool_calls，且无单工具串行选项；并行使「读 3 个文件 + 列标签」一次完成，明显改善体验。
**Migration**: runner 不再丢弃多余 tool_calls；所有 tool_calls 由 LangGraph tools 节点并行执行。`step.warning` 事件不再触发；UI 该事件的处理代码可在子项目 B 一并清理（事件协议保留以避免 renderer 解析报错）。`tool_calls` 表已是 1:N 关联 message，schema 不变。
