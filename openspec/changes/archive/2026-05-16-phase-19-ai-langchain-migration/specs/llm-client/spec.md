...
**Reason**: 由 `model.withStructuredOutput(zod)` + LangChain 内部结构化输出链路替代，不再需要手写 markdown code fence 剥离、正则抽取、Ajv 校验三段式。
**Migration**: 调用方改用 `buildChatModel(profile).withStructuredOutput(zodSchema).invoke(messages)`；解析失败由 `normalize-errors` 映射为 `E_RESPONSE`，错误码语义不变。
