...
**Reason**: 进程内 Map 的 30 分钟超时在 SqliteSaver 持久化下不再合理：app 可关闭、重启、长时间后续审批，原超时行为反而成为干扰。新设计下挂起的 interrupt 由 checkpointer 持久化，并在启动时重 emit `tool.approval-needed`（见 agent-checkpointer 规格）。
**Migration**: 30 分钟超时自动拒绝行为移除；cancel 后 thread 保留 24h（见 agent-checkpointer），超过 24h 由后台清理任务删除。用户体验：长时间未操作时审批不再自动消失，重启后仍可继续审批。
