...
function setupRig(opts?: { globalCap?: number }): Rig {
  ...
  const handlers = createChatHandlers({
    ...
    sessions,
    getTargets: () => [
      { send: (_c: string, e: AgentEvent) => events.push(e), isDestroyed: () => false }
    ],
    vaultRoot: () => vaultRoot
  })
...
describe('phase-19 acceptance: chat IPC drives new LangGraph runner (K1)', () => {
      ...
      tool_calls: [
        {
          ...
          args: { path: 'x.md', patch: { rating: 5 }, reason: 'r' }
        }
      ]
    })
    rig.agent.queueEntries([
  ...
  it('K1 ▶ HITL edit: approveTool with editedArgs becomes an `edit` decision', async () => {
      ...
      tool_calls: [
        {
          id: 'tc-e',
          name: 'update_frontmatter',
          args: { path: 'a.md', patch: { rating: 3 }, reason: 'r' }
        }
      ]
    })
    rig.agent.queueEntries([
    ...
    await rig.handlers.approveTool('tc-e', {
      editedArgs: { path: 'a.md', patch: { rating: 5 }, reason: 'r' }
    })
      ...
      tool_calls: [
        {
          id: 'tc-r',
          name: 'update_frontmatter',
          args: { path: 'a.md', patch: { rating: 1 }, reason: 'r' }
        }
      ]
    })
    rig.agent.queueEntries([
