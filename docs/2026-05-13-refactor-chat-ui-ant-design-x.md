...
export function deriveBubbleItems(messages: ChatMessage[], pendingApprovals: PendingApproval[]): BubbleItem[] {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (m.role === 'assistant' && m.toolCalls?.length) {
      ...
      out.push({
        key: m.id, role: m.role as 'user' | 'assistant',
        content: m.text,
        streaming: m.status === 'streaming',
      ...
      return (
        <>
          {c.toolSteps?.length > 0 && (
            <ThoughtChain items={c.toolSteps.map(toThoughtChainItem)} />
          )}
...
function ChatInputArea() {
  ...
  return (
    <Sender
      ...
      onSubmit={(text) => { send({ text, attachments }); setValue(''); }}
      onCancel={cancel}
      header={
...
function ApprovalInlineActions({ step }: { step: ToolStep }) {
  ...
  return (
    <>
      <Actions
        items={[
          { key: 'approve', label: t('approval.approve'), icon: <CheckOutlined />,
            onItemClick: () => approve(sessionId, callId) },
          { key: 'reject',  label: t('approval.reject'),  icon: <CloseOutlined />,
            onItemClick: () => reject(sessionId, callId) },
          { key: 'edit',    label: t('approval.edit'),    icon: <EditOutlined />,
