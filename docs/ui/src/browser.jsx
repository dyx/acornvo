      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          >
            ...
            >
              <span
                style={{ fontSize: 10.5, fontFamily: 'var(--font-mono)', color: 'var(--ink-4)' }}
              >
                https://
              </span>
              <input
              ...
              />
              <span style={{ fontSize: 10, color: 'var(--leaf)', fontFamily: 'var(--font-mono)' }}>
                ●安全
              </span>
            </div>
    ...
    >
      {state === 'idle' && (
        <>
          ...
          <span>拾果</span>
          <span
            style={{ fontFamily: 'var(--font-mono)', fontSize: 10, opacity: 0.7, marginLeft: 2 }}
          >
            ⌘⇧C
          </span>
        </>
        ...
        <>
          <div
            style={{
              ...
              borderTopColor: 'oklch(0.98 0.01 60)',
              borderRadius: '50%',
    ...
    >
      ...
      <div
        style={{
          ...
          height: 120,
          background:
            'linear-gradient(180deg, transparent 0%, oklch(0.6 0.1 50 / 0.15) 50%, transparent 100%)',
      ...
      >
        <div
          style={{
            ...
            borderTopColor: 'var(--acorn)',
            borderRadius: '50%',
            animation: 'spin 0.8s linear infinite'
          }}
        />
        <div style={{ fontSize: 11.5 }}>
          <div style={{ fontFamily: 'var(--font-serif)', color: 'var(--ink)', fontWeight: 500 }}>
            Readability 抽取正文
          </div>
          <div
            style={{
            ...
            {[0, 1, 2].map((i) => (
              <span
                ...
                style={{
                  ...
                  animation: `streamDot 1.2s ${i * 0.15}s infinite`
                }}
              />
            ))}
          </span>
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 16, lineHeight: 1.75, color: 'var(--ink-2)', marginBottom: 18 }}>
        注意力机制（<em>Attention Mechanism</em>
        ）已经成为现代深度学习架构的基石。从最初用于改进神经机器翻译的编解码瓶颈，到如今成为
        Transformer
        的核心组件，注意力机制的演进贯穿了近十年深度学习的发展脉络。本文将系统梳理它从诞生到成为主流的关键节点。
      </p>
      <h2
        style={{ fontSize: 20, fontWeight: 700, margin: '32px 0 12px', letterSpacing: '-0.005em' }}
      >
        缘起：Seq2Seq 的瓶颈
      </h2>
      <p style={{ fontSize: 16, lineHeight: 1.75, color: 'var(--ink-2)', marginBottom: 18 }}>
      ...
      <p style={{ fontSize: 16, lineHeight: 1.75, color: 'var(--ink-2)', marginBottom: 18 }}>
        同年 Bahdanau
        提出的加性注意力打破了这个瓶颈。解码器不再只看一个向量，而是对编码器所有时刻的隐状态做加权求和，权重由一个小型前馈网络计算。这一设计让模型可以"回头看"原文的任意位置。
      </p>
      <h2
        style={{ fontSize: 20, fontWeight: 700, margin: '32px 0 12px', letterSpacing: '-0.005em' }}
      >
      ...
      <p style={{ fontSize: 16, lineHeight: 1.75, color: 'var(--ink-2)', marginBottom: 18 }}>
        ...
        RNN，仅用自注意力堆叠构建编码器与解码器。这是深度学习近十年最具影响力的工作之一。
      </p>
    </div>
  )
}
...
function BookmarksDrawer({ onClose }) {
  return (
    ...
    >
      ...
      >
        <span className="serif" style={{ fontSize: 13, fontWeight: 500 }}>
          标记
        </span>
