      ...
      <TweakRow label="密度">
        ...
        >
          {['舒适', '紧凑'].map((d, i) => (
            <button
              key={d}
              onClick={() => setTweaks({ ...tweaks, density: i === 0 ? 'cozy' : 'compact' })}
              style={{
                padding: '3px 8px',
                fontSize: 10.5,
                border: 'none',
                borderRadius: 4,
                background:
                  (tweaks.density === 'cozy' && i === 0) ||
                  (tweaks.density === 'compact' && i === 1)
                    ? 'var(--paper)'
                    : 'transparent',
                color: 'var(--ink-2)',
                cursor: 'pointer',
      ...
      <TweakRow label="语言">
        ...
        >
          {['中', 'EN'].map((d, i) => (
            <button
              key={d}
              onClick={() => setTweaks({ ...tweaks, lang: i === 0 ? 'zh' : 'en' })}
              style={{
                padding: '3px 10px',
                fontSize: 10.5,
                border: 'none',
                borderRadius: 4,
                background:
                  (tweaks.lang === 'zh' && i === 0) || (tweaks.lang === 'en' && i === 1)
                    ? 'var(--paper)'
                    : 'transparent',
                color: 'var(--ink-2)',
                cursor: 'pointer',
