...
function App() {
  ...
  const openProject = (p) => { setProject(p); setScreen('browser'); };
  const nav = (s) => setScreen(s);
  const openFile = (f) => { setCurrentFile(f); setScreen('editor'); };
  ...
  return (
    <div data-screen-label={screen} style={{
      width: '100%', height: '100%', maxWidth: 1440, maxHeight: 920,
      borderRadius: 12, overflow: 'hidden',
      boxShadow: '0 0 0 0.5px oklch(0 0 0 / 0.3), 0 30px 80px oklch(0 0 0 / 0.3), 0 10px 30px oklch(0 0 0 / 0.12)',
    }}>
      {screen === 'picker' && <ProjectPicker onOpen={openProject}/>}
      {screen === 'browser' && <Browser {...commonProps} onClip={() => setReviewing(r => r + 1)}/>}
      {screen === 'library' && <Library {...commonProps} onOpenFile={openFile}/>}
      {screen === 'editor' && <Editor {...commonProps} file={currentFile} onBack={() => setScreen('library')}/>}
