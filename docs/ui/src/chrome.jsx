  ...
  return (
    <div
      style={{
        ...
        boxShadow:
          '0 0 0 0.5px oklch(0 0 0 / 0.3), 0 30px 80px oklch(0 0 0 / 0.25), 0 10px 30px oklch(0 0 0 / 0.1)',
        display: 'flex',
...
function TitleBar({ title, onClose, right, accent = 'var(--paper-2)', borderless }) {
  return (
    ...
    >
      ...
      <div style={{ minWidth: 52, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
        {right}
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
        {items.map((it) => (
          <RailBtn key={it.id} {...it} active={current === it.id} onClick={() => onNav(it.id)} />
        ))}
      </div>
...
function RailBtn({ icon, label, sub, active, onClick }) {
  return (
    ...
    >
      {icon}
      <span style={{ fontSize: 11, fontFamily: 'var(--font-serif)', fontWeight: 500 }}>
        {label}
      </span>
    </button>
  )
