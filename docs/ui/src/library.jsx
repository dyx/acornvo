      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        >
          ...
          <div style={{ padding: '2px 12px 12px', display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {['attention', 'transformer', '综述', 'LLM', '产品分析', '设计', 'electron'].map(
              (t) => (
                <span
                  key={t}
                  style={{
  ...
  return (
    <button
      ...
      style={{
        width: 'calc(100% - 16px)',
        margin: '1px 8px',
        padding: `5px 10px 5px ${10 + indent * 12}px`,
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <span
                ...
                style={{
                  ...
                  height: 6,
                  borderRadius: 1,
                  background: i < file.rating ? 'var(--acorn)' : 'var(--paper-3)',
                  border: '0.5px solid var(--line)'
                }}
              />
            ))}
    ...
    <div style={{ padding: '24px 32px', maxWidth: 640 }}>
        ...
        <div style={{ display: 'flex', gap: 2, marginBottom: 20 }}>
          {Array.from({ length: 5 }).map((_, i) => (
            <I.Star
              key={i}
              size={14}
              stroke={i < file.rating ? 'var(--acorn)' : 'var(--line-2)'}
              fill={i < file.rating ? 'var(--acorn)' : 'transparent'}
            />
          ))}
        </div>
      )}
      {file.summary ? (
        ...
        >
          ...
          <p
            className="serif"
            style={{ fontSize: 14, lineHeight: 1.75, color: 'var(--ink-2)', margin: 0 }}
          >
            {file.summary}
          </p>
          {file.highlights && (
