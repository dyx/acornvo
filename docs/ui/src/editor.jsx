  ...
  return (
    <div style={{ fontFamily: 'var(--font-serif)', color: 'var(--ink)', lineHeight: 1.8 }}>
      ...
      >
        ...
        <div>
          <span style={{ color: 'var(--acorn-2)' }}>title:</span> 深度学习中的注意力机制：从 Seq2Seq
          到 Transformer
        </div>
        <div>
          <span style={{ color: 'var(--acorn-2)' }}>url:</span>{' '}
          https://jiqizhixin.com/articles/attention-mechanism
        </div>
        <div>
          <span style={{ color: 'var(--acorn-2)' }}>rating:</span>{' '}
          <span style={{ color: 'var(--leaf)' }}>5</span>
        </div>
        <div>
          <span style={{ color: 'var(--acorn-2)' }}>category:</span> 技术/深度学习
        </div>
        <div>
          <span style={{ color: 'var(--acorn-2)' }}>tags:</span> [attention, transformer, 综述]
        </div>
      ...
      >
        <em>作者：机器之心编译组 · 发布于 2026-03-15</em>
      </p>
      ...
      <p style={{ fontSize: 16, marginBottom: 18 }}>
        2014 年，Sutskever 等人提出的 Seq2Seq 模型用两个 RNN
        分别做编码与解码。该架构有一个根本缺陷：所有输入信息被压缩进一个固定长度的上下文向量，长序列上丢失严重。
      </p>
      <blockquote
        style={{
          borderLeft: '2px solid var(--acorn)',
          margin: '0 0 18px',
      ...
      <p style={{ fontSize: 16, marginBottom: 18 }}>
        同年 Bahdanau
        提出的加性注意力打破了这个瓶颈。解码器不再只看一个向量，而是对编码器所有时刻的隐状态做加权求和，权重由一个小型前馈网络计算。
      </p>
      <h2 style={{ fontSize: 22, fontWeight: 700, margin: '28px 0 12px' }}>
        自注意力与 Transformer
      </h2>
      <p style={{ fontSize: 16, marginBottom: 18 }}>
        2017 年 Vaswani 等人的《Attention is All You Need》把注意力机制推到极致——完全抛弃
        RNN，仅用自注意力堆叠构建编码器与解码器。
      </p>
      <p style={{ fontSize: 16, marginBottom: 18 }}>
        关键创新包括 <strong>Multi-Head Attention</strong>、<strong>Position Encoding</strong> 与{' '}
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          padding: '14px 16px',
          background: 'linear-gradient(135deg, var(--acorn-bg) 0%, var(--leaf-bg) 100%)',
          borderBottom: '0.5px solid var(--line)'
        }}
      >
        <div
          ...
          {[
            ...
            <div
              ...
              style={{
                ...
                padding: '6px 8px',
                borderRadius: 6,
                background: h.current ? 'var(--acorn-bg)' : 'transparent',
                marginBottom: 4,
                fontSize: 11,
                fontFamily: 'var(--font-mono)',
                color: 'var(--ink-2)'
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {Array.from({ length: 5 }).map((_, i) => (
            <I.Star
              key={i}
              size={16}
              stroke={i < (file.rating || 0) ? 'var(--acorn)' : 'var(--line-2)'}
              fill={i < (file.rating || 0) ? 'var(--acorn)' : 'transparent'}
      ...
      <Field label="摘要">
        <p
          className="serif"
          style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--ink-2)', margin: 0 }}
        >
          {file.summary || '—'}
        </p>
      </Field>
      ...
      <Field label="来源">
        ...
        >
          <div>
            <span style={{ color: 'var(--ink-4)' }}>site</span> {file.site}
          </div>
          <div>
            <span style={{ color: 'var(--ink-4)' }}>clip</span> {file.clipped}
          </div>
          <div>
            <span style={{ color: 'var(--ink-4)' }}>review</span> {file.reviewed}
          </div>
        </div>
      </Field>
    </div>
