...
function ModelsTab() {
  return (
    <>
      <TabHeader
        title="模型预设"
        sub="API Keys via Electron safeStorage · Keychain / DPAPI / libsecret"
      />
          ...
          {USAGE_DAILY.map((d) => (
            ...
            >
              ...
              <div
                style={{
                  width: '100%',
                  height: `${(d.cost / max) * 80}px`,
                  background: 'linear-gradient(180deg, var(--acorn) 0%, var(--acorn-2) 100%)',
        ...
        {[
          ...
          <div
            ...
            style={{
              ...
              fontFamily: 'var(--font-mono)',
              color: 'var(--ink-2)',
              borderBottom: i < 2 ? '0.5px solid var(--line)' : 'none',
              alignItems: 'center'
            }}
          >
...
function GeneralTab() {
  return (
    <>
      ...
      <Row label="主题" desc="外观模式">
        <Segmented options={['浅色', '深色', '跟随系统']} active={0} />
      </Row>
      <Row label="语言" desc="界面文案">
        <Segmented options={['简体中文', 'English']} active={0} />
      </Row>
      <Row label="编辑器默认模式" desc="Vditor 三种模式之一">
        <Segmented options={['IR', 'SV', 'WYSIWYG']} active={0} />
      </Row>
      <Row label="Autosave 延迟" desc="停止输入后多久写盘">
        <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>
          2000 ms
        </span>
      </Row>
      <Row label="自动更新" desc="后台下载 · stable 通道">
        <Toggle on />
      </Row>
    </>
  )
...
function ClipperTab() {
  return (
    <>
      ...
      <Row label="下载图片到本地" desc="写入 assets/<slug>/ 并重写引用">
        <Toggle on />
      </Row>
      <Row label="采集后立即理果" desc="推入后台队列">
        <Toggle on />
      </Row>
      <Row label="伪装 Chrome UA" desc="规避部分站点的 Electron 嗅探">
        <Toggle />
      </Row>
      <Row label="正文过短兜底" desc="少于 200 字时弹选区模式">
        <Toggle on />
      </Row>
    </>
  )
...
function ReviewerTab() {
  return (
    <>
      ...
      <Row label="摘要语言" desc="跟随原文 / 强制中/英">
        <Segmented options={['跟随原文', '中文', 'English']} active={0} />
      </Row>
      <Row label="摘要字数" desc="建议值">
        <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>
          150 字
        </span>
      </Row>
      <Row label="截断阈值" desc="超长正文取前 N tokens">
        <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>
          12,000 tokens
        </span>
      </Row>
      <Row label="并发数" desc="队列同时跑几条">
        <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>
          2
        </span>
      </Row>
      <Row label="历史版本保留" desc="per-file">
        <span className="mono" style={{ fontSize: 12, color: 'var(--ink)' }}>
          5 版 · 30 天
        </span>
      </Row>
...
function AboutTab() {
  return (
    <>
      ...
      <p
        className="serif"
        style={{ fontSize: 14, lineHeight: 1.75, color: 'var(--ink-2)', maxWidth: 480 }}
      >
        一款本地优先的个人知识管理桌面应用。隐喻松鼠「拾果 · 理果 ·
        松语」的采集整理过程：浏览器拾取网页、AI 结构化加工、基于本地知识库对话生成。
      </p>
...
function Row({ label, desc, children }) {
  return (
    ...
    >
      <div style={{ flex: 1 }}>
        <div className="serif" style={{ fontSize: 13.5, color: 'var(--ink)' }}>
          {label}
        </div>
        <div
          style={{
