...
describe('migration 009_sessions', () => {
  ...
  it('tool_calls.session_id is required and indexed', () => {
    ...
    expect(() =>
      db
        .prepare('INSERT INTO tool_calls (id, tool_name, args_json) VALUES (?, ?, ?)')
        .run('tc1', 'search_files', '{}')
...
describe('agent-types', () => {
  it('Tool has required schema-driven shape', () => {
    ...
    expectTypeOf(t.execute).parameters.toEqualTypeOf<
      [{ q: string }, import('./agent-types').ToolCtx]
    >()
  })
...
vi.mock('../settings/profiles', () => ({
  profilesStore: {
    get: vi.fn(() => ({ id: 'p1', provider: 'openai', model: 'gpt-x', apiKeyRef: 'k1' }))
  }
}))
...
export async function callProviderStream(
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      Authorization: `Bearer ${req.profile.decryptedKey}`
    },
    body: JSON.stringify({
...
export async function callProviderTools(
  ...
  const toolCalls = (choice?.message?.tool_calls ?? []).map((tc: any) => {
    ...
    return {
      ...
      args: v.ok ? v.args : { __invalid: true, raw: tc.function.arguments, error: v.error }
    }
  })
  return {
...
interface ProviderStreamRequest extends ProviderRequest {
  signal?: AbortSignal
}
...
export function parseAndValidate(
  _name: string,
  raw: string,
  _tools: Array<{ name: string; parameters: any }>
) {
  try {
    return { ok: true as const, args: JSON.parse(raw) }
  } catch (e) {
    return { ok: false as const, error: 'E_INVALID_JSON' }
  }
}
```
  ...
  it('parses NDJSON streaming response', async () => {
    ...
    const body = new ReadableStream<Uint8Array>({
      start(c) {
        ...
        c.enqueue(
          enc.encode(JSON.stringify({ done: true, prompt_eval_count: 1, eval_count: 2 }) + '\n')
        )
        c.close()
      }
    })
...
describe('ollama.callProviderTools', () => {
  ...
  it('falls back to JSON-parse of plain-text content when no native tool_calls', async () => {
    ...
    expect(r.finishReason).toBe('tool_calls')
    expect(r.toolCalls).toEqual([
      { id: expect.any(String), name: 'search_files', args: { query: 'x' } }
    ])
  })
  ...
  for (;;) {
    ...
    while ((nl = buf.indexOf('\n')) >= 0) {
      ...
      if (j.message?.content) {
        ...
        hooks.onToken(j.message.content)
      }
      if (j.done)
        usage = { promptTokens: j.prompt_eval_count ?? 0, completionTokens: j.eval_count ?? 0 }
    }
  }
  return { text, usage, latencyMs: Date.now() - t0, model: req.model ?? req.profile.model }
}
...
export async function callProviderTools(
  ...
  const j = await res.json()
  const usage =
    j.prompt_eval_count !== undefined
      ? { promptTokens: j.prompt_eval_count, completionTokens: j.eval_count ?? 0 }
      : undefined
  ...
  if (Array.isArray(native) && native.length > 0) {
    ...
    return {
      text: j.message?.content || undefined,
      toolCalls,
  ...
  if (parsed) {
    return {
      text: undefined,
      toolCalls: [{ id: `ol_${randomUUID()}`, name: parsed.tool, args: parsed.args }],
      finishReason: 'tool_calls',
      usage,
      latencyMs: Date.now() - t0,
      model: req.model ?? req.profile.model
...
function tryParseFallback(content: string): { tool: string; args: unknown } | null {
  ...
  for (const line of lines) {
    ...
    try {
      const j = JSON.parse(line)
      if (j && typeof j.tool === 'string' && typeof j.args === 'object')
        return { tool: j.tool, args: j.args ?? {} }
    } catch {
      /* try next */
    }
  }
...
export function parseAndValidate(
  ...
  if (typeof raw === 'string') {
    try {
      ...
      return { ok: false, error: 'E_INVALID_JSON' }
    }
  } else {
    args = raw
  }
...
export function __resetValidatorCacheForTest() {
  cache.clear()
}
```
