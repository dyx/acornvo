...
const mockApi = {
  chat: {
    sessions: {
      ...
      create: vi.fn().mockResolvedValue({ id: 's-new', title: '未命名对话', createdAt: 1, updatedAt: 1, profileId: null })
    },
    onChatStream: vi.fn(() => () => {})
  }
};
...
describe('Chat acceptance — 11.3', () => {
  it('rAF batching — token events accumulate; one flush per frame', async () => {
    ...
    mockApi.chat.onChatStream = vi.fn((cb: any) => { streamHandler = cb; return () => {}; });
    useChatStore.setState({
      sessions: [{ id: 's1', title: 'A', createdAt: 1, updatedAt: 1, profileId: null }],
      activeSessionId: 's1',
      bySession: { s1: { loaded: true, messages: [], streamingBuffer: '', flushedLength: 0, pendingApprovals: [], pendingAttachments: [], pendingPromptText: '', lastUserText: '', lastUserAttachments: [], status: 'streaming', error: null } }
    });
...
describe('Chat acceptance — 11.4', () => {
  it('Esc during streaming → cancelStream → idle status; preserve emitted text', async () => {
    ...
    useChatStore.setState({
      sessions: [{ id: 's1', title: 'A', createdAt: 1, updatedAt: 1, profileId: null }],
      activeSessionId: 's1',
      bySession: { s1: { loaded: true, messages: [], streamingBuffer: '已写', flushedLength: 0, pendingApprovals: [], pendingAttachments: [], pendingPromptText: '', lastUserText: '', lastUserAttachments: [], status: 'streaming', error: null } }
    });
    render(<ChatInput />);
    ...
    streamHandler!({ sessionId: 's1', type: 'done', message: { id: 'm1', role: 'assistant', text: '已写', createdAt: 1 } });
...
beforeEach(() => {
  ...
  useChatStore.setState({
    sessions: [{ id: 's1', title: 'A', createdAt: 1, updatedAt: 1, profileId: null }],
    activeSessionId: 's1',
    bySession: { s1: { loaded: true, messages: [], streamingBuffer: '', flushedLength: 0, pendingApprovals: [], pendingAttachments: [], pendingPromptText: '', lastUserText: '', lastUserAttachments: [], status: 'idle', error: null } }
  });
  vi.clearAllMocks();
});
afterEach(() => cleanup());
...
describe('ChatAttachments acceptance — 11.5', () => {
  it('typing @ + picking opens QuickSwitcher and inserts chip; send carries attachment', async () => {
    ...
    expect(mockApi.chat.sendUserMessage).toHaveBeenCalledWith(expect.objectContaining({
...
const mockApi = {
  chat: {
    sessions: {
      list: vi.fn().mockResolvedValue([{ id: 's1', title: 'A', createdAt: 1, updatedAt: 1, profileId: null }]),
      messages: vi.fn().mockResolvedValue([])
    },
    onChatStream: vi.fn(),
...
describe('ChatApproval acceptance — 11.6', () => {
  it('approval-needed → panel slides in with diff; approve calls approveTool with original args', async () => {
    ...
    mockApi.chat.onChatStream = vi.fn((cb: any) => { h = cb; return () => {}; });
    render(<MemoryRouter><Chat /></MemoryRouter>);
    await screen.findByTestId('chat-main');
    installChatStreamSubscriber();
    ...
    expect(mockApi.chat.approveTool).toHaveBeenCalledWith({ sessionId: 's1', callId: 'c1', editedArgs: undefined });
...
describe('ChatApproval acceptance — 11.7', () => {
  it('queue 3 approvals; counter shows 2 remaining; sequential processing', async () => {
    ...
    for (const id of ['c1', 'c2', 'c3']) {
      h!({ sessionId: 's1', type: 'tool.approval-needed', callId: id, toolName: 'write_file', args: { path: `${id}.md`, content: 'x' }, reason: '' });
    }
...
const mockApi = {
  chat: {
    sessions: { list: vi.fn().mockResolvedValue([{ id: 's1', title: 'A', createdAt: 1, updatedAt: 1, profileId: null }]), messages: vi.fn().mockResolvedValue([]) },
    onChatStream: vi.fn(() => () => {}),
...
describe('ChatErrors acceptance — 11.9', () => {
  it('second send while streaming → toast', async () => {
    ...
    await userEvent.keyboard('{Meta>}{Enter}{/Meta}');
    expect(toastSpy).toHaveBeenCalledWith(expect.objectContaining({ title: expect.stringMatching(/已在生成|already generating/i) }));
  });
});
```
...
describe('Chat acceptance — 11.10', () => {
  it('A streaming, current=B → A row shows pulsing dot; clears on done', async () => {
    ...
    h!({ sessionId: 'sA', type: 'done', message: { id: 'mA', role: 'assistant', text: 'x', createdAt: 1 } });
...
describe('Chat acceptance — 11.11', () => {
  it('search filters; right-click rename + delete via dialog', async () => {
    ...
    // Delete
    await userEvent.pointer({ keys: '[MouseRight>]', target: screen.getAllByTestId('session-row')[0] });
...
describe('Chat acceptance — 11.12', () => {
  it('on remount, sessions + persisted messages restore; streamingBuffer is clean', async () => {
    ...
    const { unmount } = render(<MemoryRouter><Chat /></MemoryRouter>);
    await screen.findByTestId('chat-main');
...
describe('ChatAttachments acceptance — 11.14', () => {
  it('missing file → 读取失败 block; collectAttachmentContext does not throw', async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'a17-'));
    const out = await collectAttachmentContext(
      [{ type: 'file', path: 'missing.md', title: 'M' }],
      { groveRoot: tmp, clipsGet: vi.fn() }
    );
...
describe('Chat acceptance — 11.15', () => {
  it('completed assistant message renders markdown elements', () => {
    useChatStore.setState({
      ...
      bySession: {
        s1: {
          loaded: true, messages: [
            { id: 'm1', role: 'assistant', text: '- item1\n- item2\n\n```\ncode\n```\n\n| a | b |\n| - | - |\n| 1 | 2 |\n', createdAt: 1 }
          ],
          streamingBuffer: '', flushedLength: 0, pendingApprovals: [], pendingAttachments: [], pendingPromptText: '', lastUserText: '', lastUserAttachments: [], status: 'idle', error: null
        }
      }
    });
    render(<MessageList />);
  ...
  it('streaming uses pre-wrap (no markdown parsing)', () => {
    useChatStore.setState({
      ...
      activeSessionId: 's1',
      bySession: {
        s1: { loaded: true, messages: [], streamingBuffer: '**bold**', flushedLength: 0, pendingApprovals: [], pendingAttachments: [], pendingPromptText: '', lastUserText: '', lastUserAttachments: [], status: 'streaming', error: null }
      }
    });
    render(<MessageList />);
    const pre = screen.getByTestId('streaming-pre');
...
describe('Chat acceptance — 11.16', () => {
  it('clicking https link calls ipc.shell.openExternal', async () => {
    ...
    useChatStore.setState({
      sessions: [{ id: 's1', title: 'A', createdAt: 1, updatedAt: 1, profileId: null }],
      activeSessionId: 's1',
      bySession: { s1: { loaded: true, messages: [{ id: 'm1', role: 'assistant', text: 'open [example](https://example.com)', createdAt: 1 }], streamingBuffer: '', flushedLength: 0, pendingApprovals: [], pendingAttachments: [], pendingPromptText: '', lastUserText: '', lastUserAttachments: [], status: 'idle', error: null } }
    });
    render(<MessageList />);
...
describe('Chat acceptance — 11.17', () => {
  it('switching profile via top bar updates session.profileId', async () => {
    useProfilesStore.setState({
      profiles: [
        { id: 'p1', name: 'P1', provider: 'openai', model: 'gpt-4o', baseUrl: null, secretRef: null, default: false },
        { id: 'p2', name: 'P2', provider: 'anthropic', model: 'claude-opus', baseUrl: null, secretRef: null, default: false }
      ]
    } as any);
    mockApi.chat.sessions.list = vi.fn().mockResolvedValue([
      { id: 's1', title: 'A', createdAt: 1, updatedAt: 1, profileId: 'p1' }
    ]);
