...
export interface Rig {
  ...
  vaultRoot: string;
  events: any[];
  llm: { chatWithTools: ReturnType<typeof vi.fn>; queue(r: ChatWithToolsResult & { latencyMs?: number; model?: string; usage?: any }): void };
...
describe('acceptance 9.3: search → tool → answer flow with full session message log', () => {
  ...
  beforeEach(() => {
    ...
    rig.db.prepare("INSERT INTO files (path, title, content_hash, mtime, size, summary, body) VALUES (?, ?, ?, ?, ?, ?, ?)")
      .run('notes/attn.md', 'Attention', 'h1', '2026-01-01T00:00:00Z', 100, '', 'Self-attention summary.');
    rig.db.prepare("INSERT INTO files_fts(rowid, path, title, body) SELECT rowid, path, title, body FROM files").run();
  ...
  it('persists user → assistant(tool_call) → tool → assistant in session_messages', async () => {
    ...
    expect(JSON.parse(toolMsg.content!)).toMatchObject({ ok: true, data: { items: expect.any(Array) } });
...
describe('acceptance 9.8: 30-min approval timeout → E_APPROVAL_TIMEOUT', () => {
  ...
  afterEach(() => { vi.useRealTimers(); rig.cleanup(); });
...
vi.mock('../../services/db', () => ({ dbService: { requireCurrent: vi.fn(), getCurrent: vi.fn(() => ({ name: '/v' })) } }));
