...
beforeEach(() => {
  ...
  ;(settingsStore.get as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
    defaultProfileId: 'p1'
  })
...
`
    ...
    const [rel, fm, body, opts] = (
      fileHandlers.writeParsed as unknown as { mock: { calls: unknown[][] } }
    ).mock.calls[0] as [string, Record<string, unknown>, string, { expectedMtime: number }]
  ...
  it('rethrows E_MTIME_CONFLICT when writeParsed fails with E_MTIME_MISMATCH', async () => {
    ...
    ;(fileHandlers.writeParsed as unknown as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
      mtimeErr
    )
