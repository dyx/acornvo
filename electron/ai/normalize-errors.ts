...
function bucketByStatus(status: number, providerMessage?: string): NormalizedLlmError {
  if (status === 400 || status === 422) {
    return build('E_CONFIG', `bad request (HTTP ${status})`, {
      httpStatus: status,
      providerMessage
    })
  }
  ...
  if (status === 402) {
    return build(
      'E_BALANCE',
      providerMessage
        ? `Insufficient balance: ${providerMessage}`
        : `Insufficient balance (HTTP ${status})`,
      { httpStatus: status, providerMessage }
    )
  }
  if (status === 429) {
    return build(
      'E_RATE',
      providerMessage ? `Rate limited: ${providerMessage}` : `Rate limited (HTTP ${status})`,
      { httpStatus: status, providerMessage }
    )
  }
  if (status === 503) {
    return build(
      'E_RATE',
      providerMessage ? `Server busy: ${providerMessage}` : `Server busy (HTTP ${status})`,
      { httpStatus: status, providerMessage }
    )
  }
  if (status >= 500) {
