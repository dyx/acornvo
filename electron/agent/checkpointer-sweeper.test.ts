...
describe('sweepStaleThreads', () => {
  it('deletes threads canceled more than 24h ago', () => {
    ...
    expect(db.prepare("SELECT COUNT(*) AS c FROM writes WHERE thread_id='old-canceled'").get()).toEqual({ c: 0 });
    expect(db.prepare("SELECT COUNT(*) AS c FROM checkpoint_meta WHERE thread_id='old-canceled'").get()).toEqual({ c: 0 });
  ...
  it('is a no-op when no threads are stale', () => {
    ...
    expect(db.prepare("SELECT COUNT(*) AS c FROM checkpoints WHERE thread_id='a'").get()).toEqual({ c: 1 });
  });
