...
const ListFilesSchema = z.object({
  limit: z
    .number()
    .describe(
      '返回的文档最大数量。需根据用户的要求（如“20篇”、“30篇”）灵活提取，未指定时请固定传入 10。'
    ),
  offset: z.number().describe('分页偏移量。用于翻页，第一页请固定传入 0。'),
  sort_by: z
    .enum(['created_at', 'updated_at', 'title', 'size_bytes'])
    .describe(
      '排序依据。若用户想看"最新/最旧"选 created_at；"最近修改"选 updated_at；"最大/最小"选 size_bytes。'
    ),
  sort_order: z
    .enum(['DESC', 'ASC'])
...
export const listFilesTool = tool(
  async ({ limit, offset, sort_by, sort_order }) => {
    ...
    const validSortBy = ['created_at', 'updated_at', 'title', 'size_bytes'].includes(sort_by)
      ? sort_by
      : 'created_at'
