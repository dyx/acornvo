...
describe('recoverPendingApprovals', () => {
  ...
  it('re-emits tool.approval-needed for sessions with pending interrupts', async () => {
    ...
    fakeAgent.getState.mockResolvedValue({
      ...
      values: {
        messages: [{ tool_calls: [{ id: 'tc-1', name: 'update_frontmatter', args: {} }] }]
      }
    })
    ...
    const result = await recoverPendingApprovals({
      getTargets: () => [
        { send: (_c: string, e: unknown) => emitted.push(e), isDestroyed: () => false }
      ]
    })
  ...
  it('skips sessions whose profile no longer exists', async () => {
    db.prepare(
      'INSERT INTO sessions (id, title, profile_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?)'
    ).run('sess-orphan', null, 'gone', 't0', 't0')
    db.prepare(
      "INSERT INTO checkpoints (thread_id, checkpoint_ns, checkpoint_id) VALUES (?, '', 'cp-1')"
    ).run('sess-orphan')
  ...
  it('one failing thread does not block others (best-effort)', async () => {
    ...
    db.prepare(
      "INSERT INTO sessions (id, title, profile_id, created_at, updated_at) VALUES ('s-good', NULL, 'p1', 't0', 't0')"
    ).run()
    db.prepare(
      "INSERT INTO sessions (id, title, profile_id, created_at, updated_at) VALUES ('s-bad', NULL, 'p1', 't0', 't0')"
    ).run()
    db.prepare(
      "INSERT INTO checkpoints (thread_id, checkpoint_ns, checkpoint_id) VALUES (?, '', 'cp-1')"
    ).run('s-good')
    db.prepare(
      "INSERT INTO checkpoints (thread_id, checkpoint_ns, checkpoint_id) VALUES (?, '', 'cp-2')"
    ).run('s-bad')
        ...
        tasks: [
          {
            interrupts: [
              {
                id: 'int-x',
                value: { actionRequests: [{ name: 'update_frontmatter', args: { reason: 'r' } }] }
              }
            ]
          }
        ],
        values: {
          messages: [{ tool_calls: [{ id: 'tc-good', name: 'update_frontmatter', args: {} }] }]
        }
      }
    })
