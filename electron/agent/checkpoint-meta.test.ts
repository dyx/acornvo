...
describe('checkpoint-meta', () => {
  ...
  it('markThreadActive clears canceled_at when re-running on a canceled thread', () => {
    ...
    markThreadActive('t2');
    const row = db.prepare('SELECT canceled_at FROM checkpoint_meta WHERE thread_id = ?').get('t2') as {
      canceled_at: number | null;
    };
    expect(row.canceled_at).toBeNull();
  });
  ...
  it('markThreadCanceled stamps canceled_at without clobbering last_active_at', () => {
    markThreadActive('t3');
    const beforeRow = db.prepare('SELECT last_active_at FROM checkpoint_meta WHERE thread_id = ?').get('t3') as {
      last_active_at: number;
    };
    markThreadCanceled('t3');
    const afterRow = db.prepare('SELECT last_active_at, canceled_at FROM checkpoint_meta WHERE thread_id = ?').get('t3') as {
      last_active_at: number;
      canceled_at: number | null;
    };
