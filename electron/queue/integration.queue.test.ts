...
describe('Acceptance 10.2 — clip ai-review-clip enqueued', () => {
  it('enqueueing ai-review-clip via the store reflects in the jobs table', () => {
    ...
    store.enqueue('ai-review-clip', { clipId: 1, path: 'inbox/202604/a.md' }, { dedupeKey: 'clip:1' })
    const row = db.prepare("SELECT kind, status FROM jobs LIMIT 1").get() as { kind: string; status: string }
...
describe('Acceptance 10.3 — ai-review-clip handler retry on E_RATE', () => {
  ...
  it('handler catches E_RATE → retry 60s, attempts=1, status=pending', async () => {
    ...
    const row = db.prepare('SELECT status, attempts, next_run_at FROM jobs WHERE id=?').get(id) as { status: string; attempts: number; next_run_at: string }
...
describe('Acceptance 10.4 — dedupeKey idempotency', () => {
  it('second enqueue with same kind + dedupeKey returns existing id and does not insert', () => {
    const { db, store } = freshFixture()
    const a = store.enqueue('ai-review-clip', { clipId: 7, path: 'inbox/a.md' }, { dedupeKey: 'clip:7' })
    const b = store.enqueue('ai-review-clip', { clipId: 7, path: 'inbox/a.md' }, { dedupeKey: 'clip:7' })
  ...
  it('two transient EIOs then success → status=done, attempts=2', async () => {
    ...
    runner.register({
      ...
      handler: createIndexRetryHandler({
        upsertFromFs: async () => { calls++; if (calls < 3) throw new Error('EIO transient') }
      })
    })
    const { id } = store.enqueue('index-retry', { path: 'a.md' })
    runner.start()
    await vi.advanceTimersByTimeAsync(7_000)
    const row = db.prepare('SELECT status, attempts FROM jobs WHERE id=?').get(id) as { status: string; attempts: number }
...
describe('Acceptance 10.7 — jobs.retry resets attempts', () => {
  it('failed → retry → pending, attempts=0', async () => {
    const { db, store } = freshFixture()
    const handlers = createJobsHandlers({ getStore: () => store, cancelInRunner: () => ({ error: 'E_STATUS_NOT_ALLOWED' as const }) })
    const { id } = store.enqueue('index-retry', { path: 'x.md' })
    store.markRetry(id, 0, 'a'); store.markRetry(id, 0, 'b'); store.markRetry(id, 0, 'c'); store.markRetry(id, 0, 'd')
    store.markFailed(id, 'gave up')
    const before = db.prepare('SELECT attempts, status FROM jobs WHERE id=?').get(id) as { attempts: number; status: string }
    expect(before.attempts).toBe(4)
    expect(before.status).toBe('failed')
    const r = await handlers.retry(id)
    expect(r).toEqual({ ok: true })
    const after = db.prepare('SELECT attempts, status FROM jobs WHERE id=?').get(id) as { attempts: number; status: string; next_run_at: string }
...
describe('Acceptance 10.8 — clearDone removes done, preserves failed', () => {
  it('returns { removed } and leaves failed rows alone', async () => {
    const { db, store } = freshFixture()
    const handlers = createJobsHandlers({ getStore: () => store, cancelInRunner: () => ({ ok: true as const }) })
    const a = store.enqueue('index-retry', { path: 'a.md' }); const b = store.enqueue('index-retry', { path: 'b.md' })
    const c = store.enqueue('index-retry', { path: 'c.md' }); const d = store.enqueue('index-retry', { path: 'd.md' })
    store.markDone(a.id); store.markDone(b.id); store.markDone(c.id)
...
describe('Acceptance 10.12 — concurrency cap = 2 for ai-review-clip', () => {
  ...
  it('with 5 enqueued, only 2 are running concurrently', async () => {
    ...
    runner.register({
      kind: 'ai-review-clip', concurrency: 2, minGapMs: 0,
      handler: ({ job }) => new Promise<{ kind: 'ok' }>((resolve) => {
        inFlight.add(job.id); maxInFlight = Math.max(maxInFlight, inFlight.size)
        release.push(() => { inFlight.delete(job.id); resolve({ kind: 'ok' }) })
      })
    })
    ...
    while (release.length) { release.shift()!(); await vi.advanceTimersByTimeAsync(150) }
    expect(maxInFlight).toBe(2)
    const done = (db.prepare("SELECT COUNT(*) AS n FROM jobs WHERE status='done'").get() as { n: number }).n
    expect(done).toBe(5)
    db.close()
  })
})
