...
describe('handleTelemetryAggregate', () => {
  ...
  it('inserts aggregated rows into telemetry_local', () => {
    ...
    db.prepare(
      `INSERT INTO ai_usage (prompt_tokens, completion_tokens, created_at) VALUES (100, 50, '2026-05-09T12:00:00Z')`
    ).run()
    db.prepare(
      `INSERT INTO ai_usage (prompt_tokens, completion_tokens, created_at) VALUES (20, 10, '2026-05-09T18:00:00Z')`
    ).run()
    db.prepare(
      `INSERT INTO clips (url, created_at) VALUES ('http://a.com', '2026-05-09T08:00:00Z')`
    ).run()
    db.prepare(
      `INSERT INTO perf_samples (ts, area, ok, ms) VALUES ('2026-05-09T09:00:00Z', 'search.query', 1, 120)`
    ).run()

    handleTelemetryAggregate({ db, day })

    const rows = db
      .prepare('SELECT metric, value FROM telemetry_local WHERE day = ? ORDER BY metric')
      .all(day) as { metric: string; value: number }[]
    expect(rows).toEqual([
  ...
  it('upserts on conflict (re-run for same day)', () => {
    ...
    const count = (
      db.prepare('SELECT COUNT(*) AS n FROM telemetry_local WHERE day = ?').get(day) as {
        n: number
      }
    ).n
    expect(count).toBe(4)
    const value = (
      db
        .prepare('SELECT value FROM telemetry_local WHERE day = ? AND metric = ?')
        .get(day, 'ai.requests') as { value: number }
    ).value
    expect(value).toBe(1)
  })
})
