...
export async function hybridSearch(
  ...
  const ftsStmt = db.prepare(`
    SELECT 
      chunk_id, 
      bm25(files_fts, 0.0, 5.0, 10.0, 1.0) as raw_score 
    FROM files_fts
  ...
  if (vs && vecWeight > 0) {
    try {
      ...
      let vecQuery: number[]
      if (isLocal) {
        vecQuery = await embedQueryLocal(query)
      } else {
  ...
  const hydrateStmt = db.prepare(
    `SELECT chunk_id, path, heading_path, body FROM chunks WHERE chunk_id IN (${placeholders})`
  )
  const rows = hydrateStmt.all(...ids) as {
    chunk_id: string
    path: string
    heading_path: string
    body: string
  ...
  const ftsSnippets = hydrateSnippets(db, ftsQuery, ids)
  ...
  const items = top
    .map((t) => {
      ...
      let bodySnippet = ''
      if (hasFts) {
        bodySnippet = ftsSnippets.get(t.chunk_id) || ''
      }
      if (!bodySnippet) {
        bodySnippet = cleanFtsSnippet(truncateForPreview(r.body, 300))
      }
  ...
  return { items, total: items.length, pending: false }
}
