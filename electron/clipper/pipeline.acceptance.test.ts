  ...
  it('extract degraded=true → preview.degraded=true', async () => {
    const p = createPipeline({
      extract: { extract: vi.fn(async () => ({
        ok: true, degraded: true, title: 'T', content: '<p>x</p>', url: 'https://x/', lang: 'en'
      } as any)) },
...
describe('OpenSpec 9.12 — GFM table', () => {
  it('table → pipe-style markdown', () => {
    const html = '<table><thead><tr><th>Name</th><th>Score</th></tr></thead><tbody><tr><td>A</td><td>1</td></tr></tbody></table>'
...
describe('OpenSpec 9.18 — ops_log + enqueue', () => {
  it('saveClip success calls opsLog and enqueue', async () => {
    ...
    const p = createPipeline({
      ...
      indexUpsert: vi.fn(),
      clipsDao: {
        create: vi.fn(async () => ({ id: 7, url: 'x', path: 'p', title: 'X', site: 'x', author: null, publishedAt: null, clippedAt: '', excerpt: null, contentLength: null, degraded: false, createdAt: '' })),
